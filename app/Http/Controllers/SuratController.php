...
class SuratController extends Controller
{
    ...
    public function indexDalam(Request $request)
    {
            ...
            // }
                ...
                $surat = suratTugas::all();
            $penugasan = Penugasan::with(['surat' => function ($query) {
                $query->select('*')
                    ->with([
                        'pegawai',
                        'peran' => function ($queryPeran) {
                            $queryPeran->orderBy('sort_order', 'asc');
                        }
                    ]);
                }])
                ->where('tanggalterbitSurat','like','%'.session('tahun').'%')->orderBy('Tanggalsurat','DESC')->orderBy('noSurat','DESC')->paginate(5);

                $penugasan->getCollection()->transform(function ($item) {
                    $peoples = [];
                    foreach ($item->surat as $surat_tugas) {
                        $peoples[] = [
                            'name' => $surat_tugas->pegawai->nama_karyawan,
                            'peran' => $surat_tugas->peran->nama,
                            'nip' => $surat_tugas->pegawai->nip,

                            'sort' =>$surat_tugas->peran->sort_order,
                        ];
                    }
    ...
    public function CreateperjalananDalam()
    {
        ...
        $pe    = Pegawai::orderBy('nama_karyawan','ASC')->get();
        $K     = Kegiatan::all();
        $peran = Peran::all();
        return view('admin.suratTugasdalam_create',compact('obrik','jp','pe','K','peran'));
    }
    public function store(Request $request)
    {
        ...
        foreach ($request->input('tugas') as $id_peran => $value) {
            ...
            $s = suratTugas::where('id_karyawan',$id_karyawan)->whereRaw("((ta <= '$ta' AND tp >= '$ta') OR (ta >= '$ta' AND ta <= '$tp'))")->get();
            if ($s->count() > 0) {
                # code...
                $jumlahdobeljob += 1;
                $s = $s->last();
                $dobeljob .= '<br> - Pegawai '.$s->pegawai->nama_karyawan.' sudah di tugaskan '.$s->surat->jenis->nama.' ke  '.$s->surat->obrik->nama.' pada tanggal '.$s->ta.' s/d '.$s->tp.'.';
            }
        ...
        if ($jumlahdobeljob > 0) {
            # code...
            // dd($request->all());
            return redirect('perjalananDalam_create')->with('danger', $dobeljob)->withInput();
    ...
    public function update(Request $request, $id)
    {
        ...
        if ($request->input('ubahtugas')) {
            foreach ($request->input('ubahtugas') as $id_suratTugas => $value) {
                ...
                $tp = $value['tp'];
                $id_karyawan = $value['id_karyawan'];
                $s = suratTugas::where('id_karyawan',$id_karyawan)->whereRaw("((ta <= '$ta' AND tp >= '$ta') OR (ta >= '$ta' AND ta <= '$tp')) AND id != '$id_suratTugas' ")->get();
                if ($s->count() > 0) {
                    # code...
                    $jumlahdobeljob += 1;
                    $s = $s->last();
        ...
        //  dd($jumlahdobeljob2);
         if ($jumlahdobeljob > 0) {
            # code...
            return redirect('perjalananDalam/'.$id.'/edit')->with('danger', $dobeljob)->withInput();
        }
    ...
    public function indexBaru(Request $request)
    {
        ...
        return view('admin.arsip',compact('surat','penugasan','obrik','jp','filterid_obrik','filterid_jenis_pengawasan','filterbulan','filtertahun'));
