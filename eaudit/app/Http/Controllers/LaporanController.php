...
class LaporanController extends Controller
{
       ...
       $spreadsheet = new Spreadsheet();
        $sheet       = $spreadsheet->getActiveSheet();
         ...
         // Buat sebuah variabel untuk menampung pengaturan style dari header tabel
            ...
            // Buat header tabel nya pada baris ke 3
            $sheet->setCellValue('A5', "NO"); // Set kolom A3 dengan tulisan "NO"
            $sheet->setCellValue('B5', "Uraian"); // Set kolom B3 dengan tulisan "NIS"
            $sheet->setCellValue('C5', "Obrik"); // Set kolom C3 dengan tulisan "NAMA"
            $sheet->setCellValue('D5', "Sub Kegiatan"); // Set kolom E3 dengan tulisan "TELEPON"
            $sheet->setCellValue('E5', "Bulan"); // Set kolom F3 dengan tulisan "ALAMAT"
            $sheet->setCellValue('F5', "Jumlah Laporan"); // Set kolom A3 dengan tulisan "NO"
            $sheet->setCellValue('G5', "Anggaran"); // Set kolom B3 dengan tulisan "NIS"
            $sheet->setCellValue('H5', "Tanggal Pemeriksaan"); // Set kolom C3 dengan tulisan "NAMA"
            // $sheet->setCellValue('F5', "Tanggal Penugasan"); // Set kolom F3 dengan tulisan "ALAMAT"
            // Apply style header yang telah kita buat tadi ke masing-masing kolom header
            ...
            $currentQuarter = $request->triwulan;
            $startMonth = ($currentQuarter - 1) * 3 + 1; // Bulan awal kuartal
              $endMonth = $startMonth + 2; // Bulan akhir kuartal
          $penugasan=Penugasan::whereYear('TanggalSurat', session('tahun'))
                  ->whereMonth('TanggalSurat', '>=', $startMonth)
                  ->whereMonth('TanggalSurat', '<=', $endMonth)
                  ->where("id_anggaran", '=', $request->id_anggaran)
                  ->orderBy('id_anggaran', 'ASC')
    ...
    public function rkpd()
    {
        ...
        $surat = Penugasan::whereYear('TanggalSurat', session('tahun'))
        ->whereMonth('TanggalSurat', '>=', $startMonth)
        ->whereMonth('TanggalSurat', '<=', $endMonth)
        ->orderBy('id_anggaran', 'ASC')
        ->orderBy('TanggalSurat', 'ASC')
