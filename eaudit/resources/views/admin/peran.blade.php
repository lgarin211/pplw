    ...
    <div class="table-responsive">
        <table class="table table-striped">
            ...
            <tbody class="table-border-bottom-0">
                @foreach ($peran as $index => $v)
                    <tr>
                        <td>{{ $index + $peran->firstItem() }}</td>
                        <td>{{ $v->nama }}</td>
                        <td>{{ $v->tarif }}</td>
                        <td>
                            <div class="row">
                                <div class="col-sm-4 mb-2">
                                    <a href="{{ url('Peran_baru/' . $v->id . '/edit') }}"
                                        class="btn btn-outline-primary btn-sm">Edit</a>
                                </div>
                                <div class="col-sm-4 ">
                                    <form action="{{ url('Peran_baru/' . $v->id . '/hapus') }}" method="POST"
                                        class="d-inline mb-3">
                                        @method('delete')
                                        @csrf
        ...
        <div class="d-flex mt-3 justify-content-end">{{ $peran->links() }}</div>
    </div>
