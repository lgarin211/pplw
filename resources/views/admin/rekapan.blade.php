                        ...
                        <td>
                            <table>
                                <tr>
                                    ...
                                    <td>
                                        @foreach ($v->surat as $item)
                                            <?php echo isset($item->pegawai->nama_karyawan)?$item->pegawai->nama_karyawan:' ' ?> <br>
                                        @endforeach
                                    </td>
                                    <td>
                                        @foreach ($v->surat as $item)
          ...
          <?php $no = 1; ?><?php $no1 = 1; ?> <?php $no2 = 1; ?><?php $no3 = 1; ?>
            ...
            @foreach ($peran as $pr)
                ...
                @foreach ($surat1[$bulan][$pr->id]->groupBy('pegawai.nama_karyawan') as $item)
                    ...
                    <td><?php echo isset($item->first()->pegawai->nama_karyawan)?$item->first()->pegawai->nama_karyawan:' ' ?> : {{  $item->sum('perhitungan_hari') }} Hari <br><br> </td>
            ...
            <table border="1px" style="width: 100%;" class="me-5">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>                        <?php echo isset($pg->first()->pegawai->nama_karyawan)?$pg->first()->pegawai->nama_karyawan:' ' ?></td>
                        ...
                        @endphp
                        @foreach ($peran as $p)
                            <td>{{ $jumlahperan = $pg->where('id_peran',$p->id)->sum('PerhitunganHari') }}</td>
                        @php
        ...
        <div class="page">
            ...
            @endphp
                @foreach ($surat[$bulan]->groupBy('nama_karyawan') as $p)
                    ...
                    <tr>
                        <td><?php echo isset($p->first()->pegawai->nama_karyawan)?$p->first()->pegawai->nama_karyawan:' ' ?></td>
